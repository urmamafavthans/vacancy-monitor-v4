...
function cleanDetailTitle(value) {
  return normalizeTitle(cleanText(value).replace(/^(?:vacature|vacancy|job)\s*[:\-–—]?\s*/i, ''));
}
function resolvedTitle(candidate, h1, documentTitle, finalUrl) {
  const original = normalizeTitle(candidate.title);
  const detailPage = !sameUrl(finalUrl, candidate.sourceUrl);
  if (!detailPage) return original;
  const fromH1 = cleanDetailTitle(h1);
  if (fromH1 && !isGenericNavigationTitle(fromH1)) return fromH1;
  const fromDocument = cleanDetailTitle(String(documentTitle || '').split(/\s+[|–—-]\s+/)[0]);
  return fromDocument || original;
}
export function verifyJobPage(candidate, page) {
  const originalTitle = normalizeTitle(candidate.title);
  ...
  const title = resolvedTitle(candidate, h1, documentTitle, finalUrl);
  ...
  const detailPage = !sameUrl(finalUrl, candidate.sourceUrl);
  const relevant = detailPage
    ? cleanText(mainText)
    : cleanText(candidate.inlineSectionText || candidate.employmentText || mainText);
