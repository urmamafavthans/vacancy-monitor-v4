...
import { SHEETS, URL_MASTER_HEADERS, VACANCY_LOG_HEADERS } from './config.js';
...
export async function readUrlMaster(client) {
  const { data } = await client.spreadsheets.values.get({ spreadsheetId: spreadsheetId(), range: `${SHEETS.URL_MASTER}!A1:O500`, valueRenderOption: 'FORMATTED_VALUE' });
  ...
  return rows.slice(1).map((row, index) => ({ row, rowNumber: index + 2 })).filter(({ row }) => row.some((cell) => String(cell ?? '').trim() !== '')).map(({ row, rowNumber }) => ({
    rowNumber, enabled: String(row[0] ?? '').trim().toLowerCase() === 'yes', institution: String(row[1] ?? '').trim(), entryUrl: String(row[2] ?? '').trim(), strategy: String(row[3] ?? 'AUTO').trim() || 'AUTO', crawlDepth: Number(row[4] ?? 2) || 2, resolvedVacancyUrl: String(row[5] ?? '').trim(), allowedDomain: String(row[6] ?? '').trim(), jobUrlPattern: String(row[7] ?? '').trim(), adapter: String(row[8] ?? 'AUTO').trim() || 'AUTO', lastChecked: String(row[9] ?? '').trim(), status: String(row[10] ?? '').trim(), activeVacancies: Number(row[11] ?? 0) || 0, notes: String(row[12] ?? '').trim(), city: String(row[13] ?? '').trim(), travelTimeMinutes: String(row[14] ?? '').trim(),
  }));
}
export async function setConfigValue(client, key, value) {
  const { data } = await client.spreadsheets.values.get({ spreadsheetId: spreadsheetId(), range: `${SHEETS.CONFIG}!A1:B100`, valueRenderOption: 'FORMATTED_VALUE' }); const rows = data.values ?? []; const rowIndex = rows.findIndex((row) => String(row[0] ?? '').trim() === key); if (rowIndex < 0) throw new Error(`CONFIG key not found: ${key}`);
  await client.spreadsheets.values.update({ spreadsheetId: spreadsheetId(), range: `${SHEETS.CONFIG}!B${rowIndex + 1}`, valueInputOption: 'RAW', requestBody: { values: [[value]] } });
}
